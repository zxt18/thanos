...
## Warning: Only One Singleton has to run against single Object Storage.

Because there is no safe locking mechanism for all object storage provides, currently you need to on your own ensure that only
single Compactor is running against single bucket. Running more can result with [Overlap Issues](../operating/troubleshooting.md#overlaps)
that has to be resolved manually.

Singleton, means also that there should be no local compaction on Prometheus side that could potentially cause both compacted and non compacted blocks
being uploaded by sidecar. This is why uploaded flag is still under a separate `--shipper.upload-compacted` flag that helps to ensure that compacted blocks are uploaded before anything else.
The singleton rule is also why local Prometheus compaction has to be disabled in order to use sidecar with upload option. Use hidden `--shipper.ignore-unequal-block-size` to override this check (on your own risk).
...
The only acceptable case of running multiple Compactors against single Bucket is when you want to [scale compaction process](#scalability).
...
index quite well in most cases, because usually series live longer than duration of the smallest blocks, so 2 hours. It really depe
...
This is very common while using [Receivers](../components/receive.md) with replication greater than 1.
